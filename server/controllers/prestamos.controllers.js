...
export const prestamosUsuario = async (req, res) => {
    ...
    try {
        const [rows] = await pool.query( 'SELECT u.id_usuario AS "ID", u.nombre_usuario AS "USUARIO", u.identificacion_usuario AS "IDENTIFICACION", COUNT(p.id_usuario) AS "PRESTAMOS" FROM prestamos AS p INNER JOIN usuarios AS u ON p.id_usuario = u.id_usuario WHERE u.id_usuario = ? GROUP BY u.nombre_usuario, u.identificacion_usuario;', [id_usuario]);
        res.status(200).json(rows)
    } catch (error) {
        res.status(500).json({
            status: "error",
